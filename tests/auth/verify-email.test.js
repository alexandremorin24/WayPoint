...
describe('📨 GET /api/backend/verify-email', () => {
  ...
  it('should verify email with valid token', async () => {
    const res = await request(app).get(`/api/backend/auth/verify-email?token=${token}`);
    expect(res.statusCode).toBe(302); // redirection vers front
  });

  it('should return 400 with invalid token', async () => {
    const res = await request(app).get('/api/backend/auth/verify-email?token=invalid.token.here');
  ...
  it('should return 404 for unknown user', async () => {
    ...
    const res = await request(app).get(`/api/backend/auth/verify-email?token=${fakeToken}`);
